...
import { getProjects, getEmployees, getTeams, getProjectStats, createProject, deleteProject } from '@/lib/data-service';
...
type View = 'dashboard' | 'project' | 'calculator' | 'employees' | 'teams';
...
export function Dashboard() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      const [projectsData, employeesData, teamsData] = await Promise.all([
        getProjects(),
        getEmployees(),
        getTeams(),
      ]);
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="card card-body">
            ...
            <p className="text-2xl font-semibold text-red-600">
              {Array.from(projectStats.values()).reduce((sum, s) => sum + s.violationCount, 0)}
            </p>
          </div>
        </div>
