        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body space-y-4">
            ...
            <div>
              ...
              <input id="name" type="text" required value={name} onChange={e => setName(e.target.value)} className="form-input" placeholder="e.g., Night Shift A" />
            </div>
            <div>
              <label className="form-label">Team Members ({selectedIds.length} selected)</label>
                ...
                {employees.length === 0 ? (
                  ...
                  employees.map(employee => (
                    <label key={employee.id} className="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                      <input type="checkbox" checked={selectedIds.includes(employee.id)} onChange={() => toggleEmployee(employee.id)} className="mr-3" />
                      <div>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" disabled={loading} className="btn btn-primary">{loading ? 'Creating...' : 'Create Team'}</button>
          </div>
        </form>
      </div>
    </div>
