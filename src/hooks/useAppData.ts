...
export function useAppData(organisationId: string | null): UseAppDataReturn {
  ...
  const createFatigueAssessment = async (assessment: FatigueAssessment) => {
    ...
    const { error } = await supabase.from('fatigue_assessments').insert({
      ...
      job_title: assessment.jobTitle || null,
      contract_no: assessment.contractNo || null,
      location: assessment.location || null,
      assessment_date: assessment.assessmentDate,
      // Convert empty strings to null for time fields (Postgres requires null or valid time)
      shift_start_time: assessment.shiftStartTime || null,
      shift_end_time: assessment.shiftEndTime || null,
