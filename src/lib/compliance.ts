  ...
  for (const checkDate of uniqueDates) {
    ...
    for (let i = 0; i < windowAssignments.length; i++) {
      ...
      const shift = {
        ...
        commuteIn: assignment.commuteIn ?? (pattern.commuteTime ? Math.floor(pattern.commuteTime / 2) : 20),
        commuteOut: assignment.commuteOut ?? (pattern.commuteTime ? Math.ceil(pattern.commuteTime / 2) : 20),
