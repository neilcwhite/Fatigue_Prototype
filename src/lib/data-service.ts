...
export async function createAssignments(assignments: Omit<Assignment, 'id' | 'created_at' | 'updated_at'>[]): Promise<Assignment[]> {
  const { data, error } = await supabase
    .from('assignments')
    .insert(assignments)
    .select();
...
export async function deleteAssignments(ids: number[]): Promise<void> {
  const { error } = await supabase
    .from('assignments')
    .delete()
    .in('id', ids);
...
export async function getProjectStats(projectId: number): Promise<ProjectStats> {
  ...
  assignments.forEach(a => {
    ...
    if (pattern) {
      let startTime = a.custom_start_time || pattern.start_time;
      let endTime = a.custom_end_time || pattern.end_time;
      
      // Check weekly schedule
      if (!startTime && pattern.weekly_schedule) {
...
export async function assignTeamToShift(
  teamId: number,
  projectId: number,
  ...
  // Create assignments for each member and date
  const assignments: Omit<Assignment, 'id' | 'created_at' | 'updated_at'>[] = [];
  
  team.member_ids.forEach(employeeId => {
    dates.forEach(date => {
      assignments.push({
  ...
  return createAssignments(assignments);
}
